...
function stringFormatter(typeValue, schema) {
  ...
  if (minLength && maxLength) {
    value = `${value}(${minLength}, ${maxLength})`;
  }
...
function primitive (schema) {
  ...
  var formatter = primitivesFormatter[type];
  if (formatter) {
    value = formatter(value, schema);
  }
...
var parser = function (url, opts) {
  ...
  return SwaggerParser.dereference(url).then(function (parsedSpec) {
    var spec = { ...parsedSpec } // make a copy
